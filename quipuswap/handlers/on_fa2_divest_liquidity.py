...
from quipuswap.types.quipu_fa2.parameter.divest_liquidity import (
    DivestLiquidityParameter,
)
...
async def on_fa2_divest_liquidity(
    ...
    token_qty = sum(Decimal(tx.amount) for tx in transfer.parameter.__root__[0].txs) / (
        10**decimals
    )
